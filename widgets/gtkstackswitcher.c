...
#define GTK_STACK_SWITCHER_GET_PRIVATE(obj) \
	(G_TYPE_INSTANCE_GET_PRIVATE \
	((obj), GTK_TYPE_STACK_SWITCHER, GtkStackSwitcherPrivate))
...
gtk_stack_switcher_init (GtkStackSwitcher *switcher)
{
  ...
  switcher->priv = GTK_STACK_SWITCHER_GET_PRIVATE (switcher);

  //priv = gtk_stack_switcher_get_instance_private (switcher);
  priv = GTK_STACK_SWITCHER_GET_PRIVATE (G_OBJECT(switcher));
                   ...
                   GtkStackSwitcher *self)
{
  ...
  priv = GTK_STACK_SWITCHER_GET_PRIVATE (G_OBJECT(self));
               ...
               const gchar *title)
{
  ...
  //button_child = gtk_button_new ();
  //gtk_widget_set_halign (button_child, GTK_ALIGN_START);
  //gtk_widget_set_hexpand (button_child, TRUE);

  if (title != NULL) {
    gtk_widget_set_tooltip_text (GTK_WIDGET (self), title);
    //gtk_button_set_label (GTK_BUTTON (button_child), title);
    gtk_button_set_label (GTK_BUTTON (self), title);
  }

  if (icon_name != NULL ) {
    /*gtk_button_set_image (GTK_BUTTON (button_child), 
  ...
  if (button_child)
    {
      //gtk_button_set_relief (GTK_BUTTON (button_child), GTK_RELIEF_NONE);
      //gtk_widget_show_all (button_child);
      //gtk_container_add (GTK_CONTAINER (self), button_child);
      //gtk_box_pack_start( GTK_CONTAINER (self), button_child, TRUE, FALSE, 0);
    }
}
